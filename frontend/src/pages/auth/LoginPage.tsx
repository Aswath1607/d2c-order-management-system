  ...
  return (
    <div className="grid overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-300/30 md:grid-cols-2">
      <div className="relative overflow-hidden bg-slate-950 p-8 text-white md:p-12">
        <div className="relative z-10"><span className="brand-lockup bg-white p-2"><img src="/branding/aurevia-logo-exact.png" alt="Aurevia" className="h-10 w-auto object-contain" /></span><h1 className="mt-16 text-4xl font-extrabold tracking-tight">Your store,<br /><span className="text-indigo-400">in motion.</span></h1>
      ...
      <form className="p-8 md:p-12" onSubmit={handleSubmit}>
        ...
        <div className="space-y-4">
          <label className="block text-sm font-semibold text-slate-700">Email<input type="email" autoComplete="username" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="mt-2 h-11 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 outline-none transition focus:border-indigo-400 focus:bg-white" /></label>
          <label className="block text-sm font-semibold text-slate-700">Password<input type="password" autoComplete="current-password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Your password" className="mt-2 h-11 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 outline-none transition focus:border-indigo-400 focus:bg-white" /></label>
