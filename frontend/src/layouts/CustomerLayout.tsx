    ...
    <div className="min-h-screen bg-[#f7f8fc] text-slate-800">
      <header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 lg:px-6">
          <Link to="/customer" className="flex items-center gap-2 text-xl font-extrabold tracking-tight text-slate-900"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-600 text-sm text-white">D2</span> D2C</Link>
          <nav className="flex items-center gap-1 overflow-x-auto">
            ...
            <button onClick={() => { logout(); addToast('Logged out successfully', 'info'); }} className="ml-2 rounded-lg bg-slate-900 px-3 py-2 text-sm font-semibold text-white transition hover:bg-slate-700">Logout</button>
          </nav>
        </div>
      </header>
