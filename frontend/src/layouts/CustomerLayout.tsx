...
export default function CustomerLayout() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fc] text-slate-800 dark:bg-slate-950 dark:text-slate-200">
      <header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/90 backdrop-blur dark:border-slate-700 dark:bg-slate-900/90">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 lg:px-6">
          <Link to="/customer" className="brand-lockup"><img src="/branding/aurevia-mark-exact.png" alt="Aurevia" className="h-7 w-7 object-contain sm:hidden" /><img src="/branding/aurevia-logo-exact.png" alt="Aurevia" className="hidden h-8 w-auto object-contain sm:block" /></Link>
          <nav className="flex items-center gap-1 overflow-x-auto">
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
                className={({ isActive }) => `whitespace-nowrap rounded-lg px-3 py-2 text-sm font-semibold transition-colors ${isActive ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-200' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800 dark:hover:bg-slate-800 dark:hover:text-slate-100'}`}
              >
